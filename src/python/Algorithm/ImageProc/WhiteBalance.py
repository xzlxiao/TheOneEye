...
def white_balance_1(img):
    ...
    # 读取图像
    b, g, r = cv2.split(img)
    r_avg = cv2.mean(r)[0] + minor_num
    g_avg = cv2.mean(g)[0] + minor_num
    b_avg = cv2.mean(b)[0] + minor_num
    # 求各个通道所占增益
...
def white_balance_2(img_input):
    ...
    avg_b = (sum_b / time)  + minor_num
    avg_g = (sum_g / time)  + minor_num
    avg_r = (sum_r / time)  + minor_num
...
def white_balance_5(img):
    ...
    avl_r = (sum_r / num_rgb)  + minor_num
    avl_g = (sum_g / num_rgb)  + minor_num
    avl_b = (sum_b / num_rgb)  + minor_num
